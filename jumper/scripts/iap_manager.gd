...
func _ready():
	...
	if Engine.has_singleton("InAppStore"):
		...
		if result == OK:
			MyUtility.add_log_msg("Successfully started product info request")
...
func purchase_skin():
	if google_payment:
		...
		if result == OK:
			MyUtility.add_log_msg("Purchase result is OK!")
		else:
			...
			match event.type:
				...
				"purchase":
					if event.product_id == apple_product_id:
						...
						MyUtility.add_log_msg("Purchased the skin IAP, unlocking it!")
				
				"restore":
					MyUtility.add_log_msg("Restore event: transaction_id: " + event.transaction_id + ", product_id: " + event.product_id)
					if event.product_id == apple_product_id:
						unlock_new_skin.emit()
